...
if (!function_exists('bdthemes_services_carousel')) {
	function bdthemes_services_carousel($atts){
		...
		if( $wp_query->have_posts() ) :
			...
			$output[] = '<div class="bdt-services-carousel style-'.esc_attr($style).'"">';
			...
			while ( $wp_query->have_posts() ) : $wp_query->the_post();
		  		$saved_meta = BDT_SERVICES_FIELDS(get_the_ID());
		  		$service_thumbnail= wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'service' );

		  		$output[] = '<div class="bdt-carousel-slide">';

		  		if($service_thumbnail[0] != ''){
		  
		  		$output[] = '<a href="'.esc_url(get_permalink()).'" title="' . esc_attr(get_the_title()) . '" class="service-pic"><img src="'.esc_url($service_thumbnail[0]).'" alt="' . esc_attr(get_the_title()) . '" /><span class="service-overlay"></span><i class="fa fa-align-left"></i></a>';
		  		
		  		}
		  		
		  		$output[] = '<div class="service-item-description">
								<h4><a href="'.esc_url(get_permalink()).'" title="' . esc_attr(get_the_title()) . '">'.esc_html(get_the_title()) .'</a></h4>';
								if ($duration != 'false') {
									$output[] = '<span class="service-duration">'.$saved_meta['duration'].'</span>';
								}
						  		if ($excerpt != 'false') {
	...
	add_shortcode('bdt_services_carousel', 'bdthemes_services_carousel');
}