...
class mtsGoogleTypography {
	...
	function help_tab() {
		...
		$adding_title					= __('Adding A Collection', 'splash' );
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', 'splash' ).'</p>';
		$adding_content			 .= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', 'splash' ).'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', 'splash' ).'</a></p>';
		$customizing_title 		= __('Customizing', 'splash' );
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', 'splash' ).'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', 'splash' ).'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', 'splash' ).'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', 'splash' ).'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', 'splash' ).'</li><li><b>'.__('Font Variant', 'splash' ).'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', 'splash' ).'</li><li><b>'.__('Font Size', 'splash' ).'</b> - '.__('The size you would like this font to be.', 'splash' ).'</li><li><b>'.__('Font Color', 'splash' ).'</b> - '.__('The color you\'d like to use for this font.', 'splash' ).'</li><li><b>'.__('CSS Selectors', 'splash' ).'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', 'splash' ).'</li></ul>';
		$selectors_title 			= __('CSS Selectors', 'splash' );
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', 'splash' ) . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', 'splash' ) . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', 'splash' ).':</b> '.__('#selector', 'splash' ).'</li><li><b>'.__('Classes:', 'splash' ).':</b> '.__('.selector', 'splash' ).'</li><li><b>'.__('HTML Tags', 'splash' ).':</b> '.__('span', 'splash' ).'</ul>';
		$selectors_content	 .= '<p><b>'.__('Example', 'splash' ).':</b> '.__('#selector span.date', 'splash' ).'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', 'splash' ) );
		}
		return $links;
	}
	...
	function options_ui() {
		...
		$title								= __('Theme Typography', 'splash' );
		$loading							= __('Loading Your Collections', 'splash' );
		$add_new					= __('Add New Collection', 'splash' );
		$reset						= __('Reset Collections', 'splash' );
		$preview_text   			= __('Type in some text to preview...', 'splash' );
		$collection_title 			= __('New Collection', 'splash' );
		//$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', 'splash' );
		//$font_family_title  	= __('Font family...', 'splash' );
		$std_font_family_title  	= __('Standard Fonts', 'splash' );
		$google_font_family_title  	= __('Google Fonts', 'splash' );
		$font_variant_title 	= __('Variant...', 'splash' );
		$font_size_title 			= __('Size...', 'splash' );
		$backup_font_title		  = __('Fallback:', 'splash' );
		$css_selectors_title	= __('CSS Selectors:', 'splash' );
		$additional_css_title	= __('Additional CSS:', 'splash' );
		$delete_button_text   = __('Delete', 'splash' );
		$save_button_text			= __('Save', 'splash' );
		
		$welcome_title 				= __('Theme Typography', 'splash' );
		$welcome_subtitle			= __('Get started in 3 steps.', 'splash' );//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', 'splash' ).'</a>';
		$step_1_title 				= __('1. Pick A Font', 'splash' );
		$step_1_desc					= __('Choose from any of the 600+ Google Fonts.', 'splash' );
		$step_2_title 				= __('2. Customize It', 'splash' );
		$step_2_desc					= __('Pick a size, variant, color and more.', 'splash' );
		$step_3_title 				= __('3. Attach It', 'splash' );
		$step_3_desc					= __('Attach your font to any CSS selector(s).', 'splash' );
		$year									= date("Y");
		$themename = MTS_THEME_NAME;
		
		$character_sets_title = __('Character sets', 'splash' );
		$character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', 'splash' );
		...
		$character_sets = array(
			'latin' => __('Latin', 'splash' ),
			'latin-ext' => __('Latin Extended', 'splash' ),
			'cyrillic' => __('Cyrillic', 'splash' ),
			'cyrillic-ext' => __('Cyrillic Extended', 'splash' ),
			'greek' => __('Greek', 'splash' ),
			'greek-ext' => __('Greek Extended', 'splash' ),
			'vietnamese' => __('Vietnamese', 'splash' ),
			'khmer' => __('Khmer', 'splash' ),
			'devanagari' => __('Devanagari', 'splash' ),
		);
...
EOT;
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
				'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", 'splash' ),
				'delete_confirm' => __("Are you sure you want to delete this collection?", 'splash' )
			));
