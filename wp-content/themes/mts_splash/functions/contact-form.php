	...
	class mtscontact {
		...
		public function ajax_mtscontact() {
			if ($this->validate()) {
				if ($this->send_mail()) {
					...
					$this->errors['sendmail'] = __('An error occurred. Please contact site administrator.', 'splash' );
		...
		private function validate() {
			...
			if (!check_ajax_referer( 'mtscontact', 'mtscontact_nonce', false )) {
				$this->errors['nonce'] = __('Please try again.', 'splash' );
			}
			
			// check honeypot // must be empty
			if (!empty($_POST['mtscontact_captcha'])) {
				$this->errors['captcha'] = __('Please try again.', 'splash' );
			}
			
			// name field
			$name = trim(str_replace(array("\n", "\r", "<", ">"), '', strip_tags($_POST['mtscontact_name'])));
			if (empty($name)) {
				$this->errors['name'] = __('Please enter your name.', 'splash' );
			}
			
			// email field
			$useremail = trim($_POST['mtscontact_email']);
			if (!is_email($useremail)) {
				$this->errors['email'] = __('Please enter a valid email address.', 'splash' );
			}
			
			// message field
			$message = strip_tags($_POST['mtscontact_message']);
			if (empty($message)) {
				$this->errors['message'] = __('Please enter a message.', 'splash' );
			}
		...
		private function send_mail() {
			...
			$email_subject = __('Contact Form Message from', 'splash' ).' '.get_bloginfo('name');
			$email_message = __('Name:', 'splash' ).' '.$this->userinput['name']."\n\n".
							 __('Email:', 'splash' ).' '.$this->userinput['email']."\n\n".
							 __('Message:', 'splash' ).' '.$this->userinput['message'];
		...
		public function get_form() {
			...
			if (!$this->success) {
				...
				<label for="mtscontact_name">'.__('Name', 'splash' ).'</label>
				<input type="text" name="mtscontact_name" value="'.esc_attr($this->userinput['name']).'" id="mtscontact_name" />
				
				<label for="mtscontact_email">'.__('Email', 'splash' ).'</label>
				<input type="text" name="mtscontact_email" value="'.esc_attr($this->userinput['email']).'" id="mtscontact_email" />
				
				<label for="mtscontact_message">'.__('Message', 'splash' ).'</label>
				<textarea name="mtscontact_message" id="mtscontact_message">'.esc_textarea($this->userinput['message']).'</textarea>
				
				<input type="submit" value="'.__('Send', 'splash' ).'" id="mtscontact_submit" />
			</form>';
			}
			$return .= '<div id="mtscontact_success"'.($this->success ? '' : ' style="display: none;"').'>'.__('Your message has been sent.', 'splash' ).'</div>';
			return $return;
		}
