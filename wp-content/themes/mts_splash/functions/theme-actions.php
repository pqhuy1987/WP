...
if (!function_exists('mts_the_breadcrumb')) {
	...
	function mts_the_breadcrumb() {
		...
		echo '"><i class="fa fa-home"></i> '.esc_html__( "Home", 'splash' );
		echo '</a></div>&nbsp;/&nbsp;';
		if (is_single()) {
			...
			if ( $categories ) {
				...
				if ( empty( $hierarchy_arr ) ) {
					...
					echo '<div typeof="v:Breadcrumb"><a href="'. esc_url( get_category_link( $category->term_id ) ).'" rel="v:url" property="v:title">'.esc_html( $category->name ).'</a></div>&nbsp;/&nbsp;';
				} else {
					foreach ( $hierarchy_arr as $cat_id ) {
						$category = get_term_by( 'id', $cat_id, 'category' );
						echo '<div typeof="v:Breadcrumb"><a href="'. esc_url( get_category_link( $category->term_id ) ).'" rel="v:url" property="v:title">'.esc_html( $category->name ).'</a></div><span class="cat_bull"> </span>';
					}
				}
			}
			echo "<div><span>";
			...
			if ( $parent_id ) {
				...
				while ( $parent_id ) {
					...
					$breadcrumbs[] = '<div typeof="v:Breadcrumb"><a href="'.esc_url( get_permalink( $page->ID ) ).'" rel="v:url" property="v:title">'.esc_html( get_the_title($page->ID) ). '</a></div>&nbsp;/&nbsp;';
...
function mts_the_category( $separator = ', ' ) {
	...
	foreach ( $categories as $i => $category ) {
		echo '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" title="' . sprintf( __( "View all posts in %s", 'splash' ), esc_attr( $category->name ) ) . '">' . esc_html( $category->name ).'</a>';
		if ( $i < $count - 1 )
			echo $separator;
	}
}
...
function mts_the_tags($before = '', $sep = ', ', $after = '</div>') {
	if ( empty( $before ) ) {
		$before = '<div class="tags border-bottom">'.__('Tags: ', 'splash' );
	}
...
if (!function_exists('mts_pagination')) {
	...
	function mts_pagination($pages = '', $range = 3) {
		$mts_options = get_option(MTS_THEME_NAME);
		if (isset($mts_options['mts_pagenavigation']) && $mts_options['mts_pagenavigation'] == '1' ) { // numeric pagination
			the_posts_pagination( array(
				'mid_size' => 3,
				'prev_text' => '<i class="fa fa-angle-left"></i> ' . __( 'Previous', 'splash' ),
				'next_text' => __( 'Next', 'splash' ) . ' <i class="fa fa-angle-right"></i>',
			) );
		} else { // traditional or ajax pagination
			?>
			<div class="pagination pagination-previous-next">
			<ul>
				<li class="nav-previous"><?php next_posts_link( '<i class="fa fa-angle-left"></i> '. __( 'Previous', 'splash' ) ); ?></li>
				<li class="nav-next"><?php previous_posts_link( __( 'Next', 'splash' ).' <i class="fa fa-angle-right"></i>' ); ?></li>
			</ul>
			</div>
			<?php
		}
...
<div class="mts-cart">
	...
	<span>
		<?php if ( is_user_logged_in() ) { ?>
			<a href="<?php echo esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ); ?>" title="<?php _e('My Account', 'splash' ); ?>"><i class="fa fa-user"></i>&nbsp;<?php _e('My Account', 'splash' ); ?></a>
		<?php } 
		else { ?>
			<a href="<?php echo esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ); ?>" title="<?php _e('Login / Register', 'splash' ); ?>"><i class="fa fa-user"></i>&nbsp;<?php _e('Login ', 'splash' ); ?></a>
		<?php } ?>
	</span>
	<span>
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php _e('View your shopping cart', 'splash' ); ?>"><i class="fa fa-shopping-cart"></i>&nbsp;<?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'splash' ), $woocommerce->cart->cart_contents_count);?> - <?php echo $woocommerce->cart->get_cart_total(); ?></a>
	</span>
</div>
<?php } 
	}
	...
	function mts_related_posts() {
		...
		//if(!empty($mts_options['mts_related_posts'])) { ?>	
			...
			$my_query = new WP_Query( apply_filters( 'mts_related_posts_query_args', $args, $mts_options['mts_related_posts_taxonomy'] ) ); if( $my_query->have_posts() ) {
				...
				echo '<h4>'.__('Related Posts', 'splash' ).'</h4>';
				...
				<article class="latestPost excerpt  <?php echo (++$j % $posts_per_row == 0) ? 'last' : ''; ?>">
					<a href="<?php echo esc_url( get_the_permalink() ); ?>" title="<?php echo esc_attr( get_the_title() ); ?>" id="featured-thumbnail">
						<?php echo '<div class="featured-thumbnail">'; the_post_thumbnail('splash-related',array('title' => '')); echo '</div>'; ?>
		...
		switch ( $button ) {
			...
			?>
				...
				<span class="share-item twitterbtn">
					<a href="https://twitter.com/share" class="twitter-share-button" data-via="<?php echo esc_attr( $mts_options['mts_twitter_username'] ); ?>"><?php esc_html_e( 'Tweet', 'splash' ); ?></a>
				</span>
			<?php
			break;
			case 'gplus':
			...
			?>
				...
				<span class="share-item pinbtn">
					<a href="http://pinterest.com/pin/create/button/?url=<?php the_permalink(); ?>&media=<?php $thumb = wp_get_attachment_image_src( get_post_thumbnail_id(get_the_ID()), 'large' ); echo $thumb['0']; ?>&description=<?php the_title(); ?>" class="pin-it-button" count-layout="horizontal"><?php esc_html_e( 'Pin It', 'splash' ); ?></a>
				</span>
			<?php
			break;
			case 'linkedin':
		...
		<header>
			<?php if (empty($mts_options['mts_full_posts'])) { ?>
				<a href="<?php echo esc_url( get_the_permalink() ); ?>" title="<?php echo esc_attr( get_the_title() ); ?>" class="post-image post-image-left">
					<?php echo '<div class="featured-thumbnail">'; the_post_thumbnail('splash-featured',array('title' => '')); echo '</div>';
