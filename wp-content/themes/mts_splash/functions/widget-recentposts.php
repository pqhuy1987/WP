...
class mts_recent_posts_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			sprintf( __('%sRecent Posts', 'splash' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),
			array( 'description' => __( 'Display the most recent posts from all categories', 'splash' ) )
		);
	}
 	...
 	public function form( $instance ) {
		...
		$title = isset( $instance[ 'title' ] ) ? $instance[ 'title' ] : __( 'Recent Posts', 'splash' );
		...
		<p>
			<label for="<?php echo $this->get_field_id("author"); ?>">
				...
				<?php _e( 'Show post author', 'splash'); ?>
			</label>
		</p>
	...
	public function widget( $args, $instance ) {
		...
		$title_length = isset( $instance[ 'title_length' ] ) ? $instance[ 'title_length' ] : 7;
		...
		echo self::get_cat_posts( $qty, $author, $title_length, $comment_num, $date, $show_thumb2, $box_layout, $show_excerpt, $excerpt_length );
		echo $after_widget;
	}

	public function get_cat_posts( $qty, $author, $title_length, $comment_num, $date, $show_thumb2, $box_layout, $show_excerpt, $excerpt_length ) {
		...
		while ( $posts->have_posts() ) { $posts->the_post(); ?>
			<?php echo $open_li_item; ?>
				...
				<div class="post-title">
					<?php //$titlelength = isset( $title_length ) ? $title_length : '7' ?>
