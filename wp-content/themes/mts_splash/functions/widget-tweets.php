...
class mts_widget_recent_tweets extends WP_Widget {
	public function __construct() {
		parent::__construct(
			...
			sprintf( __( '%sLatest Tweets', 'splash' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),// Name
			array( 'description' => __( 'A widget for showing latest tweets in sidebar', 'splash' ), ) // Args
		);
	}
	public function widget($args, $instance) {
		...
		if ( empty( $tweets ) ) { _e( 'Couldn\'t retrieve tweets! Wrong username?', 'splash' ); echo $after_widget; return; }
		if(!empty($tweets->errors)){
			if($tweets->errors[0]->message == 'Invalid or expired token'){
				echo '<strong>'.$tweets->errors[0]->message.'!</strong><br />'. esc_html( 'You\'ll need to regenerate it <a href="https://dev.twitter.com/apps" target="_blank">here</a>!', 'splash' ) . $after_widget;
	...
	echo '<p><label>Title:</label><input type="text" name="'.$this->get_field_name( 'title' ).'" id="'.$this->get_field_id( 'title' ).'" value="'.esc_attr($instance['title']).'" class="widefat" /></p><p><label>'.__('API Key', 'splash' ).':</label><input type="text" name="'.$this->get_field_name( 'consumerkey' ).'" id="'.$this->get_field_id( 'consumerkey' ).'" value="'.esc_attr($instance['consumerkey']).'" class="widefat" /></p><p><label>'.__('API Secret', 'splash' ).':</label><input type="text" name="'.$this->get_field_name( 'consumersecret' ).'" id="'.$this->get_field_id( 'consumersecret' ).'" value="'.esc_attr($instance['consumersecret']).'" class="widefat" /></p><p><label>'.__('Access Token', 'splash' ).':</label><input type="text" name="'.$this->get_field_name( 'accesstoken' ).'" id="'.$this->get_field_id( 'accesstoken' ).'" value="'.esc_attr($instance['accesstoken']).'" class="widefat" /></p>
	<p><label>'.__('Access Token Secret', 'splash' ).':</label><input type="text" name="'.$this->get_field_name( 'accesstokensecret' ).'" id="'.$this->get_field_id( 'accesstokensecret' ).'" value="'.esc_attr($instance['accesstokensecret']).'" class="widefat" /></p><p><label>'.__('Cache Tweets in every', 'splash' ).':</label><input type="text" name="'.$this->get_field_name( 'cachetime' ).'" id="'.$this->get_field_id( 'cachetime' ).'" value="'.esc_attr($instance['cachetime']).'" class="small-text" /> hours</p><p><label>'.__('Twitter Username', 'splash' ).'</label>
	<input type="text" name="'.$this->get_field_name( 'username' ).'" id="'.$this->get_field_id( 'username' ).'" value="'.esc_attr($instance['username']).'" class="widefat" /></p><p><label>'.__('Number of tweets (max 20)', 'splash' ).'</label><select type="text" name="'.$this->get_field_name( 'tweetstoshow' ).'" id="'.$this->get_field_id( 'tweetstoshow' ).'">';
	...
	public function relative_time($a) {			
		...
		$week = $day * 7;				
		if(is_numeric($d) && $d > 0) {				
			if($d < 3) return __( "right now", 'splash' ); //if less then 3 seconds
			if($d < $minute) return sprintf( __("%s seconds ago", 'splash' ), floor($d) ); //if less then minute
			if($d < $minute * 2) return __( "about 1 minute ago", 'splash' ); //if less then 2 minutes
			if($d < $hour) return sprintf( __( "%s minutes ago", 'splash' ), floor( $d / $minute ) ); //if less then hour
			if($d < $hour * 2) return __( "about 1 hour ago", 'splash' ); //if less then 2 hours
			if($d < $day) return sprintf( __("%s hours ago", 'splash' ), floor( $d / $hour ) ); //if less then day
			if($d > $day && $d < $day * 2) return __( "yesterday", 'splash' ); //if more then day, but less then 2 days
			if($d < $day * 365) return sprintf(  __( "%s days ago", 'splash' ), floor( $d / $day ) ); //if less then year
			return __( "over a year ago", 'splash' ); //else return more than a year
		}
	}
}	
// register	widget
