...
if ( ! isset( $content_width ) ) {
	$content_width = 610; //article content width without padding
}
...
function mts_after_setup_theme() {
	...
	add_image_size( 'splash-featured', 200, 200, true ); //featured
	add_image_size( 'splash-widgetthumb', 80, 80, true ); //widget
	add_image_size( 'splash-widgetfull', 324, 200, true ); //widget
	add_image_size( 'splash-related', 75, 75, true ); //related posts
	add_image_size( 'splash-slider', 610, 250, true ); //slider
	add_action( 'init', 'splash_wp_review_thumb_size', 11 );

	function splash_wp_review_thumb_size() {
		add_image_size( 'wp_review_small', 80, 80, true );
		add_image_size( 'wp_review_large', 324, 200, true ); 
	}

	register_nav_menus( array(
	  'primary-menu' => __( 'Primary', 'splash' ),
	  'secondary-menu' => __( 'Secondary', 'splash' ),
	  'mobile' => __( 'Mobile', 'splash' )
	) );
...
function mts_get_icons() {
	$icons = array(
		__( 'Web Application Icons', 'splash' ) => array(
		...
		__( 'Accessibility Icons', 'splash' ) => array(
		...
		__( 'Transportation Icons', 'splash' ) => array(
		...
		__( 'File Type Icons', 'splash' ) => array(
		...
		__( 'Spinner Icons', 'splash' ) => array(
			'circle-o-notch', 'cog', 'gear', 'refresh', 'spinner'
		),
		__( 'Form Control Icons', 'splash' ) => array(
		...
		__( 'Chart Icons', 'splash' ) => array(
			'area-chart', 'bar-chart', 'bar-chart-o', 'line-chart', 'pie-chart'
		),
		__( 'Currency Icons', 'splash' ) => array(
		...
		__( 'Text Editor Icons', 'splash' ) => array(
		...
		__( 'Directional Icons', 'splash' ) => array(
		...
		__( 'Video Player Icons', 'splash' ) => array(
...
function mts_get_featured_image($post_ID) {  
	...
	if ($post_thumbnail_id) {  
		$post_thumbnail_img = wp_get_attachment_image_src($post_thumbnail_id, 'splash-widgetfull');  
...
function mts_columns_head($defaults) {
	if (get_post_type() == 'post') {
		$defaults['featured_image'] = __('Featured Image', 'splash' );
	}
...
if (!post_type_exists('reviews')) {
	register_post_type('reviews', array(
		'label' => __('Reviews', 'splash'),
		'singular_label' => __('Review', 'splash'),
		'public' => true,
		'show_ui' => $wp_review_installed,
		'capability_type' => 'post',
		'rewrite' => array('slug'=>'reviews','with_front' => false),
		'query_var' => false,
		'search_items' => __('Search Reviews', 'splash'),
		'not_found' =>  __('Nothing found', 'splash'),
		'not_found_in_trash' => __('Nothing found in Trash', 'splash'),
...
function mts_convert_reviews() {
	...
	foreach ($recent_reviews as $review_post) {
		...
		if ($old_review_score || $old_review_criteria) {
			...
			update_post_meta( $review_post->ID, 'wp_review_heading', __('Review', 'splash') );
...
function mts_update_reviews_notice() {
	...
	<div class="update-nag error">
		...
		<p><em><?php _e( 'This will make permanent changes, it is recommended to create a backup of your database before starting.', 'splash' ); ?></em></p>
...
function mts_update_reviews_success() {
	?>
	<div class="updated">
		<p><?php _e( 'Reviews successfully updated.', 'splash' ); ?></p>
	</div>
	<?php
}
...
function mts_add_proscons($review, $post_id, $type, $total, $items) {
	...
	if ($pros || $cons) {
		...
		$review .= '<div class="proscons">';
			if(get_post_meta($post_id, 'mts_pros', true)):
				$review .= '<div class="pros"><b>'. __('Pros', 'splash') .':</b>&nbsp;<span>'.get_post_meta($post_id, 'mts_pros', true).'</span></div>';
			endif;
			if(get_post_meta($post_id, 'mts_cons', true)):
				$review .= '<div class="cons"><b>'. __( 'Cons', 'splash' ) .':</b>&nbsp;<span>'.get_post_meta($post_id, 'mts_cons', true).'</span></div>';
			endif;
		$review .= '</div></div>';
	}
	...
	if (function_exists('wp_review_comment_fields') || $custom_width) {
		if ($width == 100)
...
function mts_register_sidebars() {
	...
	register_sidebar(array(
		'name' => __('Header Ad', 'splash'),
		'description'   => __( 'Appears in Header', 'splash' ),
		'id' => 'widget-header' ,
	...
	// Default sidebar
	register_sidebar( array(
		'name' => __('Sidebar', 'splash'),
		'description'   => __( 'Appears on homepage, posts and pages', 'splash' ),
		'id' => 'sidebar',
	...
	register_sidebar(array(
		'name' => __('Header Ad', 'splash'),
		'description'   => __( '728x90 Ad Area', 'splash' ),
		'id' => 'widget-header',
	...
	if ( isset( $mts_options['mts_first_footer'] ) && $mts_options['mts_first_footer']) {
		// Footer widget areas
		register_sidebar(array(
			'name' => __( 'Left Footer', 'splash' ),
			'description'   => __( 'Appears in left side of footer.', 'splash' ),
			'id' => 'footer-1',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h3 class="widget-title">',
		...
		register_sidebar(array(
			'name' => __( 'Center Footer', 'splash' ),
			'description'   => __( 'Appears in center of footer.', 'splash' ),
			'id' => 'footer-2',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h3 class="widget-title">',
		...
		register_sidebar(array(
			'name' => __( 'Right Footer', 'splash' ),
			'description'   => __( 'Appears in right side of footer.', 'splash' ),
			'id' => 'footer-3',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h3 class="widget-title">',
	...
	if ( mts_is_wc_active() ) {
		...
		register_sidebar( array(
			'name' => __('Shop Page Sidebar', 'splash' ),
			'description'   => __( 'Appears on Shop main page and product archive pages.', 'splash' ),
			'id' => 'shop-sidebar',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h3 class="widget-title">',
		...
		register_sidebar( array(
			'name' => __('Single Product Sidebar', 'splash' ),
			'description'   => __( 'Appears on single product pages.', 'splash' ),
			'id' => 'product-sidebar',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h3 class="widget-title">',
...
// Add Reviews Widget
include_once( get_theme_file_path( "functions/widget-reviews.php" ) );
...
// Add Rating
include_once( get_theme_file_path( 'functions/rating/metaboxes.php' ) );
include_once( get_theme_file_path( 'functions/rating/rating.php' ) );
...
function mts_add_scripts() {
	...
	// Cookie for Sorting
	if(!empty($mts_options['mts_sorting'])) {
	...
	if( ! empty( $mts_options['mts_pagenavigation'] ) && $mts_options['mts_pagenavigation'] >= 2 && !is_singular() ) {
		...
		wp_localize_script(
			...
			array(
				...
				'autoLoad' => $autoload,
				'i18n_loadmore' => __( 'Load More Posts', 'splash' ),
				'i18n_loading' => __('Loading...', 'splash' ),
				'i18n_nomore' => __( 'No more posts.', 'splash' )
			 )
		);
	}
...
function mts_enqueue_css() {
	...
	wp_enqueue_style( 'splash-stylesheet', get_stylesheet_uri() );
	...
	if ( $mts_sidebar_location != 'right' && ( $mts_options['mts_layout'] == 'sclayout' || $mts_sidebar_location == 'left' )) {
		...
		if( isset( $mts_options['mts_social_button_position'] ) && $mts_options['mts_social_button_position'] == 'floating' ) {
			$mts_shareit_right = '.shareit { margin: 0 630px 0; border-left: 0; }';
		}
	}
	...
	if ( ! empty( $mts_options['mts_author_comment'] ) ) {
		$mts_author = '.commentlist li.bypostauthor { background: #FAFAFA; padding: 3%; padding-bottom: 0; }
		.bypostauthor:after { content: "'.__( 'Author', 'splash' ).'"; position: absolute; right: -1px; top: -1px; padding: 1px 10px; background: #818181; color: #FFF; }';
	}
	$mts_bg = mts_get_background_styles( 'mts_background' );
	$mts_footer_background = mts_get_background_styles( 'mts_footer_background' );
	$nav_color = !empty($mts_options['mts_nav_color']) ? $mts_options['mts_nav_color'] : '#222222';
	$custom_css = "
		body {{$mts_bg}}
		#site-footer {{$mts_footer_background}}
		
		.pace .pace-progress, #mobile-menu-wrapper ul li a:hover, .ball-pulse > div, .pagination .nav-previous a:hover, .pagination .nav-next a:hover, #load-posts a:hover, .post-number, .currenttext, .pagination a:hover, .single .pagination a:hover .currenttext, .page-numbers.current, .mts-cart > span:hover, input[type='submit'], .woocommerce-product-search input[type='submit'] { background: {$mts_options['mts_color_scheme']}; }
		.postauthor h5, .copyrights a, .single_post a, .textwidget a, #logo a, .pnavigation2 a, #sidebar a:hover, .copyrights a:hover, #site-footer .widget li a:hover, .related-posts a:hover, .reply a, .title a:hover, .latestPost .title a:hover, .post-info a:hover, #tabber .inside li a:hover, .readMore a:hover, .fn a, .postauthor .fn, a, a:hover, #sidebar a, .cnumber, #site-footer .wpt_widget_content .entry-title a, #site-footer .wp_review_tab_widget_content .entry-title a { color:{$mts_options['mts_color_scheme']}; }	
		.primary-navigation .navigation li a:hover { color:{$mts_options['mts_color_scheme']}!important; }
		#commentform input#submit:hover, .contact-form input[type='submit']:hover, #move-to-top:hover, .navigation ul .current-menu-item > a, .secondary-navigation .navigation ul li:hover, #tabber ul.tabs li a.selected, .tagcloud a:hover, .navigation ul .sfHover a, .woocommerce a.button, .woocommerce-page a.button, .woocommerce button.button, .woocommerce-page button.button, .woocommerce input.button, .woocommerce-page input.button, .woocommerce #respond input#submit, .woocommerce-page #respond input#submit, .woocommerce #content input.button, .woocommerce-page #content input.button, .woocommerce .bypostauthor:after, #searchsubmit, .woocommerce nav.woocommerce-pagination ul li span.current, .woocommerce-page nav.woocommerce-pagination ul li span.current, .woocommerce #content nav.woocommerce-pagination ul li span.current, .woocommerce-page #content nav.woocommerce-pagination ul li span.current, .woocommerce nav.woocommerce-pagination ul li a:hover, .woocommerce-page nav.woocommerce-pagination ul li a:hover, .woocommerce #content nav.woocommerce-pagination ul li a:hover, .woocommerce-page #content nav.woocommerce-pagination ul li a:hover, .woocommerce nav.woocommerce-pagination ul li a:focus, .woocommerce-page nav.woocommerce-pagination ul li a:focus, .woocommerce #content nav.woocommerce-pagination ul li a:focus, .woocommerce-page #content nav.woocommerce-pagination ul li a:focus, .woocommerce a.button, .woocommerce-page a.button, .woocommerce button.button, .woocommerce-page button.button, .woocommerce input.button, .woocommerce-page input.button, .woocommerce #respond input#submit, .woocommerce-page #respond input#submit, .woocommerce #content input.button, .woocommerce-page #content input.button, .popular-posts .thecomment, #sidebar .popular-posts li:hover, .woocommerce-account .woocommerce-MyAccount-navigation li.is-active { background-color:{$mts_options['mts_color_scheme']}; color: #fff!important; }
		...
		.secondary-navigation, .mobile-menu-active .navigation.mobile-menu-wrapper, .navigation ul ul li { background: $nav_color; }
		.navigation ul ul:after { border-bottom-color: {$nav_color}; }
		{$mts_sclayout}
		{$mts_shareit_left}
		{$mts_shareit_right}
		{$mts_author}
	...
	function mts_comments( $comment, $args, $depth ) {
		...
		<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
			...
			switch( $comment->comment_type ) :
					...
					<div id="comment-<?php comment_ID(); ?>" class="comment-list">
						<div class="comment-author vcard">
							...
							<span class="comment-meta">
								<?php edit_comment_link( __( '( Edit )', 'splash' ), '  ', '' ) ?>
							</span>
						</div>
						<?php if ( $comment->comment_approved == '0' ) : ?>
							<em><?php _e( 'Your comment is awaiting moderation.', 'splash' ) ?></em>
							<br />
						<?php endif; ?>
					</div>
				<?php
				...
				default: ?>
					<div class="comment-list" id="comment-<?php comment_ID(); ?>" itemscope itemtype="http://schema.org/UserComments">
						<div class="comment-author vcard">
...
function ajax_mts_search() {
	...
	if ( !empty( $query ) && $search_query->have_posts() ) : 
			...
			?><li>
				<a href="<?php echo esc_url( get_the_permalink() ); ?>">
					<?php if ( has_post_thumbnail() ) { ?>
						<?php the_post_thumbnail( 'splash-widgetthumb', array( 'title' => '' ) ); ?>
					<?php } else { ?>
						<img class="wp-post-image" src="<?php echo get_template_directory_uri() . '/images/nothumb-splash-widgetthumb.png'; ?>" alt="<?php echo esc_attr( get_the_title() ); ?>"/>
					<?php } ?>
		...
		endwhile;
		echo '</ul>';
		echo '<div class="ajax-search-meta"><span class="results-count">'.$search_count.' '.__( 'Results', 'splash' ).'</span><a href="'.esc_url( get_search_link( $query ) ).'" class="results-link">'.__('Show all results.', 'splash' ).'</a></div>';
	else:
		echo '<div class="no-results">'.__( 'No results found.', 'splash' ).'</div>';
	endif;
	wp_reset_postdata();
...
if ( mts_is_wc_active() ) {
	...
	function mts_header_add_to_cart_fragment( $fragments ) {
		...
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php _e( 'View your shopping cart', 'splash' ); ?>"><?php echo sprintf( _n( '%d item', '%d items', $woocommerce->cart->cart_contents_count, 'splash' ), $woocommerce->cart->cart_contents_count );?> - <?php echo $woocommerce->cart->get_cart_total(); ?></a>
...
function mts_custom_gravatar( $avatar_defaults ) {
	...
	$avatar_defaults[$mts_avatar] = __( 'Custom Gravatar ( /images/gravatar.png )', 'splash' );
...
function mts_megamenu_thumbnails( $thumbnail_html, $post_id ) {
	...
	if(has_post_thumbnail($post_id)):
		$thumbnail_html .= get_the_post_thumbnail($post_id, 'splash-widgetfull', array('title' => ''));
	else:
		$thumbnail_html .= '<img src="'.get_template_directory_uri().'/images/nothumb-splash-widgetfull.png" alt="'.__('No Preview', 'splash').'"  class="wp-post-image" />';
	endif;
	$thumbnail_html .= '</a>';
...
function mts_menu_item_color( $item_output, $item_color, $item, $depth, $args ) {
	if (!empty($item_color)) {
		if ($args->theme_location == 'secondary-menu') {
			$item_output .= '<style type="text/css">#menu-item-'. $item->ID . ':hover, #menu-item-'. $item->ID . '.wpmm-megamenu-showing, #menu-item-'. $item->ID . '.current-menu-item { background: '.$item_color.'; border-color: '.$item_color.'; } 
#menu-item-'. $item->ID . ':hover + li { border-left-color: '.$item_color.' !important; }</style>';
		} else {
			$item_output .= '<style type="text/css">#navigation.primary-navigation #menu-item-'. $item->ID . ' a { color: '.$item_color.'; }</style>';
		}
	}
	return $item_output;
}
